...
export function Sections() {
	...
	const parentOptions = [
		{ label: __( 'Top level', 'doczur' ), value: '0' },
	...
	const create = async () => {
		...
		try {
			...
			setNotice( {
				status: 'success',
				message: __( 'Section created.', 'doczur' ),
			} );
		} catch ( error ) {
			setNotice( {
				status: 'error',
				message:
					error instanceof Error
						? error.message
						: __( 'Section could not be created.', 'doczur' ),
			} );
		} finally {
			setBusy( false );
		}
	...
	const save = async ( section: Section ) => {
		...
		try {
			...
			setNotice( {
				status: 'success',
				message: __( 'Section saved.', 'doczur' ),
			} );
		} catch ( error ) {
			setNotice( {
				status: 'error',
				message:
					error instanceof Error
						? error.message
						: __( 'Section could not be saved.', 'doczur' ),
			} );
		} finally {
			setBusy( false );
		}
	...
	const remove = async ( section: Section ) => {
		...
		try {
			...
			setNotice( {
				status: 'success',
				message: __( 'Section deleted.', 'doczur' ),
			} );
		} catch ( error ) {
			setNotice( {
				status: 'error',
				message:
					error instanceof Error
						? error.message
						: __( 'Section could not be deleted.', 'doczur' ),
			} );
		} finally {
			setBusy( false );
		}
				...
				<CardBody>
					<div className="itsdz-settings-card-header">
						<span
							...
							aria-hidden="true"
						/>
						<h2>{ __( 'Add section', 'doczur' ) }</h2>
					</div>
					<div className="itsdz-section-create">
						<TextControl
							label={ __( 'Name', 'doczur' ) }
							value={ name }
							onChange={ setName }
						/>
						<SelectControl
							label={ __( 'Parent', 'doczur' ) }
							value={ String( parent ) }
						...
						/>
						<TextControl
							label={ __( 'Dashicon class', 'doczur' ) }
							value={ icon }
							onChange={ setIcon }
					...
					<div className="itsdz-icon-presets" aria-label={ __( 'Icon presets', 'doczur' ) }>
