...
final class Dashboard_Widget implements Service {
	...
	public function render() {
		...
		if ( ! $kb ) {
			printf(
				'<p>%s</p>',
				esc_html__( 'No documentation project yet. Open Doczur to create one.', 'doczur' )
			);
			return;
		}
		...
		<ul class="itsdz-dashboard-health">
			<li>
				...
				<?php esc_html_e( 'drafts', 'doczur' ); ?>
			</li>
			<li>
				<strong><?php echo esc_html( (string) $stale ); ?></strong>
				<?php esc_html_e( 'need review', 'doczur' ); ?>
			</li>
			<li>
				<strong><?php echo esc_html( (string) $unsectioned ); ?></strong>
				<?php esc_html_e( 'unsectioned', 'doczur' ); ?>
			</li>
		</ul>
		<p>
			<a href="<?php echo esc_url( $screen_url ); ?>"><?php esc_html_e( 'Open documentation', 'doczur' ); ?></a>
		</p>
		<style>
